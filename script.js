...
function renderLogs() {
  ...
  filtered.sort((a, b) => {
    if (a.year !== b.year)
      return state.log.sort === "asc" ? a.year - b.year : b.year - a.year;
    return state.log.sort === "asc"
  ...
  container.innerHTML =
    filtered.length === 0
      ? '<p class="text-center opacity-50 py-20">No matching logs.</p>'
      : `
...
function renderVillagers() {
  ...
  let filtered = DATABASE.villagers.filter(
...
function renderResources() {
  ...
  let filtered = DATABASE.resourceTimeline.filter(
    (r) => r.year === state.resource.year && r.season === state.resource.season,
  );

  filtered.sort((a, b) => {
    if (state.resource.sort === "value") return b.value - a.value;
    if (state.resource.sort === "category")
  ...
  container.innerHTML =
    filtered.length === 0
      ? '<p class="col-span-2 text-center opacity-50 py-10">No records found for this period.</p>'
      : filtered
          .map(
            (r) => `
